...
export class KorbitPrivate extends KorbitShared implements IExchangePrivate {
  ...
  public async fetchBalance(): Promise<IBalance[]> {
    const result = await requestAuth<IKorbitBalance[]>(method.get, constants.apiUrl, constants.endpoints.balance, await this.header());
    return converter.balance(result);
  }

  /* ------------------입금 주소 조회-------------------- */
  public async fetchDepositAddress(currency: string): Promise<IDepositAddress> {
    const result = await requestAuth<IKorbitDepositAddress[]>(method.get, constants.apiUrl, constants.endpoints.depositAddress, await this.header());
  ...
  public async fetchDepositHistory(
    ...
    const result = await requestAuth<IKorbitDepositHistory[]>(method.get, constants.apiUrl, constants.endpoints.depositHistory, await this.header(), {
      params,
    });
