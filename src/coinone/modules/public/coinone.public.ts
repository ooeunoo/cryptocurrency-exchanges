...
export class CoinonePublic extends CoinoneShared implements IExchangePublic {
  ...
  public async fetchMarkets(): Promise<IMarket[]> {
    const result = await request<ICoinoneMarket[]>(method.get, constants.apiUrl, constants.endpoints.market);
    return converter.markets(result);
  }

  /* ------------------티커 조회-------------------- */
  public async fetchTickers(): Promise<ITicker[]> {
    const result = await request<ICoinoneTicker[]>(method.get, constants.apiUrl, constants.endpoints.ticker);
