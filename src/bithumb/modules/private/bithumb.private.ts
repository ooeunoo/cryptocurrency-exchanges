...
export class BithumbPrivate extends BithumbShared implements IExchangePrivate {
  ...
  public async fetchWalletStatus(): Promise<IWalletStatus[]> {
    const result = await request<IBithumbResponse<IBithumbWalletStatus[]>>(method.get, constants.apiUrl, constants.endpoints.walletStatus);
  ...
  public async fetchDepositHistory(
    ...
    const result = await requestAuth<IBithumbResponse<IBithumbDepositHistory[]>>(
      method.post,
      constants.apiUrl,
  ...
  public async fetchWithdrawHistory(
    ...
    const params = { searchGb: 5, order_currency: currency, payment_currency: "KRW", offset: page, count: limit };
    const result = await requestAuth<IBithumbResponse<IBithumbWithdrawHistory[]>>(
      method.post,
      constants.apiUrl,
